const bakingContainer = document.querySelector('.baking-list__cards');

fetch('json/baking.json')
    ...
    .then(jsonContent => {
        for (const categoryKey in jsonContent) {
            if (jsonContent.hasOwnProperty(categoryKey)) {
                ...
                categoryCard.classList.add('baking-list__card');
                categoryCard.style.backgroundImage = `url(${image})`;
                
                const titleElement = document.createElement('h2');
                titleElement.textContent = title;
                titleElement.classList.add('baking-list__card-title');
